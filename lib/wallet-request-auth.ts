...
export async function createSignedWalletRequestHeaders(input: {
  ...
  path: string;
  body?: string;
  signBytes: (message: any) => Promise<string>;
}) {
...
export async function verifySignedWalletRequest(input: {
  ...
  if (wallet.toLowerCase() !== input.expectedWallet.toLowerCase()) {
  ...
  try {
    ...
    throw new Error("Request authorization signature is invalid: " + err.message);
  }
